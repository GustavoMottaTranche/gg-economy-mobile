  ...
  it('should produce sequential suffixes " (1/(N-1))" through " ((N-1)/(N-1))" after removing any parcel', () => {
    fc.assert(
      fc.property(
        ...
        (groupSize, baseDescription) => {
          // Generate a removal index k (1-based, 1 ≤ k ≤ N)
          const removalIndex = fc.sample(
            fc.integer({ min: 1, max: groupSize }),
            1,
          )[0]!;
          ...
          const remaining = originalDescriptions.filter(
            (_, i) => i !== removalIndex - 1,
          );
